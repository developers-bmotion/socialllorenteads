...
<div class="d-flex flex-column flex-sm-row flex-wrap margin-bottom-6">
    <div class="col-sm-4 col-md-3 col-lg-2 pl-sm-0 d-flex justify-content-center justify-content-sm-start">
        ...
        <span class="fa-stack fa-xs source-badge-position" style="vertical-align: top;">
            <i class="fa fa-fw fa-circle text-<?= $source ?> fa-stack-2x"></i>
            <i class="<?= $language->{$source}->global->icon ?> fa-stack-1x fa-inverse"></i>
        </span>
    </div>
    ...
    <div class="col-sm-8 col-md-9 col-lg-6 pl-sm-0 d-flex justify-content-center justify-content-sm-start">
        <div class="row d-flex flex-column">
            ...
            <div class="d-flex flex-row">
                ...
                <?php if($source_account->verified): ?>
                    <span class="align-self-center ml-3" data-toggle="tooltip" title="<?= $language->facebook->report->display->verified ?>"><i class="fa fa-fw fa-check-circle user-verified-badge"></i></span>
                <?php endif ?>
            ...
            <?php if(!empty($source_account->details)): ?>
            <small class="text-muted"><span data-toggle="tooltip" title="<?= $language->facebook->report->display->type ?>"><i class="fa fa-fw fa-angle-right"></i></span> <?= $source_account->details ?></small>
            <?php endif ?>
        </div>
    </div>
    ...
    <div class="col-md-12 col-lg-4 d-flex justify-content-around align-items-center mt-4 mt-lg-0 pl-sm-0">
        <div class="col d-flex flex-column justify-content-center">
            ...
            <p class="report-header-number"><?= nr($source_account->heart) ?></p>
        </div>

        <?php if($source_account->fans > 0): ?>
        <div class="col d-flex flex-column justify-content-center">
            <?= $language->facebook->report->display->followers ?>
            <p class="report-header-number"><?= nr($source_account->fans) ?></p>
        </div>
        <?php endif ?>
    </div>
</div>
